...
# Modifed a bit by Michael Dickens cmdickens@tamu.edu
#   use FindBin and -z option added by Michael Dickens 08/31/2015
...
my $min_scaffold_length = 0; #skip scaffolds shorter than this length
 
GetOptions ("limit=i"       => \$limit,
			"csv"           => \$csv, 
			"graph"         => \$graph,
			"z=i"           => \$min_scaffold_length,
			"n=i"           => \$n_limit,
			"genome_size=i" => \$genome_size);

# set defaults
$limit = 1_000_000_000 if (!$limit);
...
options:
	...
	-z <int>     skip scaffolds/contigs less than <int> (default: 0)
...
sub process_FASTA{
	...
	while(my $entry = $fasta->nextEntry){
		...
		next if $length <= $min_scaffold_length;
		$seq_count++;
